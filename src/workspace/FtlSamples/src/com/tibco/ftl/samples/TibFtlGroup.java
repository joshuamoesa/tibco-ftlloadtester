 ...
 */
public class TibFtlGroup implements NotificationHandler
{
    String          sampleName      = "tibftlgroup";
    ...
    Realm           realm  = null;
    String          groupOrdinalChangeAdvisoryMatcherString;
    ContentMatcher  groupOrdinalChangeAdvisoryMatcher     = null;
    Subscriber      groupOrdinalChangeAdvisorySubscriber    = null;
    ...
    Group           group  = null;
    long            start;
    Message         descriptorMsg = null;
    String          clientName;

    public TibFtlGroup(String[] args)
    {
    ...
    public int work() throws FTLException
    {
        ...
        /*
         * Changes in an ordinal of a client application are reported to the application through an advisory message.
         * These advisory messages have the field "name" set to the specific value "ADVISORY_NAME_ORDINAL_UPDATE". 
         ...
         *     {"name":"ADVISORY_NAME_ORDINAL_UPDATE"}
         * which matches any message which contains a field named "name" with the specific value "ADVISORY_NAME_ORDINAL_UPDATE".
    
         * When creating the matcher string, it is good practice to use the predefined constants FIELD_NAME (defined in Advisory 
         * Interface, which is automatically included in com.tibco.ftl.*) and ADVISORY_NAME_ORDINAL_UPDATE (defined in Group 
         * Interface) rather than the literal values.
         */
        groupOrdinalChangeAdvisoryMatcher = realm.createContentMatcher("{\"" + Advisory.FIELD_NAME + "\":\"" + Group.ADVISORY_NAME_ORDINAL_UPDATE + "\"}");

        /*
         * Next, create the group ordinal change advisory subscriber object. Specify the endpoint name, the content matcher, and a properties object.
    
         * The endpoint specified is a predefined endpoint on which advisory messages are received.
         */
        groupOrdinalChangeAdvisorySubscriber = realm.createSubscriber(Advisory.ENDPOINT_NAME, groupOrdinalChangeAdvisoryMatcher, null);

        /*
         * Create instance of groupOrdinalChangeHandler which will provide the callback for handling group ordinal change advisory specifically.
         */
        groupOrdinalChangeHandler onGroupOrdinalChangeAdvisory = new groupOrdinalChangeHandler();

        /*
         * Next add the group ordinal change advisory subscriber object to the event queue. This allows the event queue to
         * dispatch messages sent to that subscriber. Specify the subscriber object, callback.
    
         * The callback must be a function with a specific signature: see the messagesReceived() method of groupOrdinalChangeHandler below
         * for an explanation of what it does.
         */
        queue.addSubscriber(groupOrdinalChangeAdvisorySubscriber, onGroupOrdinalChangeAdvisory);

        /*
         * Changes in the status of a group are also reported to the client application through an advisory message.
         * These advisory messages have the field "name" set to the specific value "ADVISORY_NAME_GROUP_STATUS". 
         * In the same way as above, create another content matcher to cause only those messages to be delivered 
        ...
        /*
         * Create the group status advisory subscriber object. Specify the endpoint name, the content matcher, and a properties object.
    
         * Note that the endpoint is the same as used when creating the group ordinal change advisory subscriber object. All
         * advisory messages are delivered to this endpoint.
         */
        groupStatusAdvisorySubscriber = realm.createSubscriber(Advisory.ENDPOINT_NAME, groupStatusAdvisoryMatcher, null);

        /*
         * Create instance of groupStatusHandler which will provide the callback for handling group status advisory specifically.
         */
        groupStatusHandler onGroupStatusAdvisory = new groupStatusHandler();
 
        /* 
         * Add the group status advisory subscriber object to the event queue. The use of separate subscribers and callbacks
         * on the same endpoint simplifies the callback logic, as only the requested messages are delivered to the callback.
         */
         ...
         * The second property is the member descriptor (PROPERTY_MESSAGE_MEMBER_DESCRIPTOR). This property value
         * is an FTL message, whose contents is left completely up to the client application designers. It may be used to
         * communicate static information about the group member to other group members. This sample uses the member descriptor
         * to provide the client name.
         */
         
        //First create the group properties.
        ...
        /*
         * Set the member descriptor property. This requires creating a message, adding one or more fields to the message,
         * then setting the actual property value.
    
         * The format used to create the message can be a dynamic format (as it is in this sample), or a static format defined
         * by the realm administrator. The field names used within the message are up to the application designers.
         */
        descriptorMsg = realm.createMessage("identifier_format");
        descriptorMsg.setString("my_descriptor_string", clientName);
        ...
        groupProps.destroy();
        // The descriptor message is also no longer required, so destroy it.
        descriptorMsg.destroy();
        ...
        // Next dispose of the group ordinal change advisory subscriber and associated objects.
        if ((queue != null) && (groupOrdinalChangeAdvisorySubscriber != null))
        {
            queue.removeSubscriber(groupOrdinalChangeAdvisorySubscriber);
            groupOrdinalChangeAdvisorySubscriber.close();
            if (groupOrdinalChangeAdvisoryMatcher != null)
            {
                groupOrdinalChangeAdvisoryMatcher.destroy();
            }
        }
    ...
    {
        TibFtlGroup  s  = new TibFtlGroup(args);
 ...
 */
class groupOrdinalChangeHandler implements SubscriberListener
{
    ...
    public void messagesReceived(List<Message> messages, EventQueue eventQueue)
    {
        ...
        try {
            ...
            {
                msg = messages.get(i);
                System.out.println("Group ordinal change advisory:");
                ...
                // Since a matcher was used when creating the group ordinal change advisory subscriber,
                // only messages in which the FIELD_NAME field contains the value
                // ADVISORY_NAME_ORDINAL_UPDATE are delivered to this callback. This helps to
                // simplify the callback code.
                svalue = msg.getString(Group.ADVISORY_FIELD_GROUP);
                System.out.println("  " + Group.ADVISORY_FIELD_GROUP + ": " + svalue);

                lvalue = msg.getLong(Group.ADVISORY_FIELD_ORDINAL);
                System.out.println("  " + Group.ADVISORY_FIELD_ORDINAL + ": " + lvalue);
            }
        }
        catch (Throwable ex)
        {
 ...
 * This class implements SubscriberListener interface which is a Message event handler to process inbound messages.
 */
class groupStatusHandler implements SubscriberListener
{
    // This callback is invoked when one or more group status advisory messages are dispatched.
    public void messagesReceived(List<Message> messages, EventQueue eventQueue)
    {
        int i;
        Message msg;
        ...
        try {
            int msgNum = messages.size();
            // A callback invocation may deliver more than one message, so loop through all messages delivered.
            for (i = 0;  i < msgNum;  i++)
            {
                svalue = "";
		msg = messages.get(i);
